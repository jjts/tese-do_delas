...
% Subfigures are handled by the 'subcaption' package loaded above.
\usepackage{graphicx}
%
% -----------------------------------------------------------------------------
% PACKAGE caption:
...
\usepackage{cleveref}
% Suppress acronym-internal labels; they are never cross-referenced and would
% otherwise cause "multiply defined" warnings when \acresetall is called in
% both abstracts.
...
% subfigure spacing (subcaption equivalents — subfigure package not used)
\captionsetup[subfigure]{skip=3pt, belowskip=2pt, margin=2pt}

%
% -----------------------------------------------------------------------------
% SIMPLE "FANCYCHAPTER" (no minitoc, no title-page chapter openers)
% Produces: "Chapter 2 - Platform Foundation" (left-aligned, one line),
% then the intro text, then immediately the first section (same page).
% -----------------------------------------------------------------------------

% Prevent forced blank pages before chapters (typical with twoside+openright)
\let\cleardoublepage\clearpage

% -----------------------------------------------------------------------------
% (REMOVED) PACKAGE minitoc:
% -----------------------------------------------------------------------------
% You said you don't want a per-chapter mini-contents anymore, so minitoc is not
% used here. If it is loaded elsewhere in your template, that's fine — we just
% won't call \minitoc anywhere.

% -----------------------------------------------------------------------------
% Keep your boxed verbatim helpers (unchanged)
% -----------------------------------------------------------------------------
\makeatletter
\def\boxedverbatim{%
...
\def\@makechapterhead#1{%
  % Start much closer to the top of the text block:
  \vspace*{-18pt}%
  {\parindent \z@ \raggedright \normalfont
    \ifnum \c@secnumdepth >\m@ne
      {\LARGE\bfseries \chaptername~\thechapter\ --\ #1\par\nobreak}%
    \else
...
% Also handle \chapter*{...} consistently
...
% -----------------------------------------------------------------------------
% \fancychapter now behaves like a normal chapter + optional intro file.
% No minitoc. No vfill. No pagebreak.
% -----------------------------------------------------------------------------
\newcommand{\fancychapter}[1]{%
  \chapter{#1}%
  \InputIfFileExists{./Chapters/fc_description_\arabic{chapter}.tex}{}{}%
}
...
% Firmware output listing style (plain monospace, no language highlighting)
\lstdefinestyle{iob_python}{%
	basicstyle=\small\ttfamily,
	numbers=none,
	breaklines=true,
	frame=single,
