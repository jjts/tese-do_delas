...
The platform includes Register M-of-N, Logic M-of-N, \ac{ST} hardware, the Fault Manager, and the \ac{CSR}-based monitoring path. The standalone \ac{FI} tool supports \ac{SEM} and \ac{ACME}-based configuration memory injection, register and \ac{FF} injection through on-chip hardware, mixed target pools, reusable area and time profiles, benchmark synchronization, and target-pool replay. The Pblock Sizing Model maps selected hardware targets to physical regions, allowing configuration memory injections to be aimed at module-level scopes.

The validation in \Cref{chap9:fatori_validation} showed that the Pblock Sizing Model reached 92.44\% average accuracy and that the implemented time profiles matched their configured behaviour with scores between 96.50\% and 98.64\%. The \ac{FTM} comparison in \Cref{chap9:general-ftm-testing} showed that register faults were the most disruptive tested fault type. Without Register M-of-N, register-injection campaigns crashed at all tested rates. With Register M-of-N and Correction enabled, the benchmark passed the full register-only sweep. Native Ibex \acp{FTM}, including SecureIbex, did not improve the breaking points in the tested \ac{SEU}-oriented campaigns.
...
Taken together, these results show that the effectiveness of a given \ac{FTM} depends critically on the fault type, the injection scope, and the specific hardware targets protected — distinctions that a generic evaluation framework cannot expose. \ac{Fatori-V} demonstrates that a single configurable platform can make these distinctions visible, enabling meaningful and reproducible \ac{FTM} comparisons on a real \ac{RISC-V} \ac{SoC} under controlled \ac{FI} conditions.
