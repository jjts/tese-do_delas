...
\ac{SRAM}-based \acp{FPGA} contain multiple memory types that may be affected by \acp{SEU} \cite{SEM,Kastensmidt2006}. Listed in order of decreasing size — and therefore decreasing probability of being hit by a random upset — they are as follows.
...
In this work, \ac{SEM} is instantiated in Mitigation and Testing mode with error classification disabled, enabling both continuous scanning and controlled fault injection; its correction features are not used.
...
\begin{figure}[h]
  ...
  \caption{SEM state transition diagram for mitigation modes \cite{SEM}.}
  \label{fig:sem_state}
\end{figure}
...
\hline
\rowcolor{tableheader}\textbf{Report} & \textbf{State} & \textbf{Meaning in practice} \\
\hline
\hline
...
\begin{tabularx}{\textwidth}{|l|l|X|}
\hline
\rowcolor{tableheader}\textbf{Command} & \textbf{Typical state} & \textbf{Effect on SEM} \\
\hline
\hline
...
The lack of complete \ac{FT}/\ac{FI} platforms noted in \Cref{chap1:motivation} is further illustrated in \Cref{tab:fi_platforms_gap}, which compares state-of-the-art platforms against the features needed to develop and test \acp{FTM}. Column headings are self-explanatory; entries use Yes, Partial, and No to indicate whether each feature is provided as part of an integrated workflow.
...
\hline
\rowcolor{tableheader}\textbf{\ac{FT}/\ac{FI} Platform} &
\textbf{Full \ac{RTL}} &
\textbf{\ac{FT} Interface} &
\textbf{Memory Types} &
\textbf{Readable \ac{UI}} &
\textbf{\ac{FI} Setup} &
\textbf{Metrics} \\
\hline
\hline
...
\hline
\rowcolor{tableheader}\textbf{Core} &
\textbf{Doc.} &
\textbf{Readability and Modification} &
\textbf{Workflow} &
\textbf{FT Infrastructure} &
\textbf{Dev. Access} &
\textbf{Min. Hardware Cost} &
\textbf{Max. Performance} &
\textbf{Community} \\
\hline
\hline
CVA6 &
