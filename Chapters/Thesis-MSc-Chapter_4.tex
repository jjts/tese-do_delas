...
VeeR EL2 \cite{veer_el2}, surveyed in \Cref{chap2:analysis}, is a 32-bit embedded \ac{RISC-V} core that served as the first \ac{Fatori-V} backbone candidate. It is delivered as a core complex with optional \ac{ICCM} and \ac{DCCM} memories, an optional instruction cache, three master ports for instruction fetch, load-store access, and debug, and one slave \ac{DMA} port for external access to core memories. The external bus protocol can be configured as 64-bit \ac{AXI} or \ac{AHB}-Lite.
...
The EL2 pipeline separates the arithmetic and control-flow path from the memory path, with \ac{DCCM} accesses including \ac{ECC} protection, stores using a write buffer, and bypass paths forwarding results to reduce stalls.

VeeRwolf adds the \ac{SoC} blocks needed to execute software around the EL2 core complex, including a boot ROM, \ac{UART}, \ac{SPI}, memory and peripheral interconnects, and an \ac{AXI}-to-Wishbone bridge. Since the available board was the Basys3 \cite{basys3}, the implementation followed the RVfpgaEL2-Basys3 course flow, which also provided simulation examples and a debug toolchain.
